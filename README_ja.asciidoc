...
[.small .blue]#*_(v1.6.0~)_*#|`DebugTrace.ILogger`    |`DebugTrace.Diagnostics.Debug` と `DebugTrace.Diagnostics.Trace` の抽象スーパークラス
  |`DebugTrace.Diagnostics.Debug` +
[.small .blue]#*_(v1.6.0~)_*#|`DebugTrace.Diagnostics`|ログを `System.Diagnostics.Debug` を使用して出力するクラス
  |`DebugTrace.Diagnostics.Trace` +
[.small .blue]#*_(v1.6.0~)_*#|`DebugTrace.Diagnostics`|ログを `System.Diagnostics.Trace` を使用して出力するクラス
|===
...
`Logger = Diagnostics+Debug` [.small .blue]#- System.Diagnostics.Debugクラスを使用して出力 *_(v1.6.0~)_*# +
`Logger = Diagnostics+Trace` [.small .blue]#- System.Diagnostics.Traceクラスを使用して出力 *_(v1.6.0~)_*# +
...
* 対象フレームを.NET Frameword 4.7から.NET Standard 2.0に変更。
